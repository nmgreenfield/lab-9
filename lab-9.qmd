...
**Code:**
The plot is made by generating a bunch of lines and assigning them to either top or bottom based on the angles. The two sets of lines will be colored differently. The main parameters that can be adjusted are the sample size (n), and the two color palettes. Increasing the sample size will increase the number of lines, which makes the plot look more dense. The palettes for both the bottom and top lines can be adjusted to customize the colors. 
...
**Code:**
The plot is made by using `fracture()`. Setting the `noise` parameter to *gen_simplex* makes the plot look like a lava lamp. There are three main parameters that can be adjusted to change the image - fps, speed, and freq. Speed changes the amount of movement in the lava lamp, with lower values being more jumpy. The freq parameter makes the blobs larger for smaller values (in reality, it just zooms out and makes things look smaller for higher values). Finally, the fps adjust the numer of frames per second in the gif. Smaller values are better for a more authentic lava lamp look. 